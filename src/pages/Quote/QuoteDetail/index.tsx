...
const MovieDetail: FC = () => {
  ...
  return <Container>
    <DetailBox>
      {character && Object.keys(quote || {})?.map((key) => quote?.[key] ? <Title key={key}>{key} - {quote?.[key]}</Title> : undefined)}
      {character && Object.keys(character || {})?.map((key) => character?.[key] ? <Title key={key}>{key} - {character?.[key]}</Title> : undefined)}
      {character && Object.keys(movie || {})?.map((key) => movie?.[key] ? <Title key={key}>{key} - {movie?.[key]}</Title> : undefined)}
    </DetailBox>
  </Container>;
};
